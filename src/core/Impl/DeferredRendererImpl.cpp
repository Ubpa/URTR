...
DeferredRenderer::Impl::Impl()
	: depth{ new gl::RenderBuffer }
{
	for (auto& tex : gtexs) {
		tex = new gl::Texture2D;
		tex->SetWrapFilter(gl::WrapMode::ClampToEdge, gl::WrapMode::ClampToEdge, gl::MinFilter::Nearest, gl::MagFilter::Nearest);
	}
	...
	default_white.SetWrapFilter(gl::WrapMode::Repeat, gl::WrapMode::Repeat, gl::MinFilter::Nearest, gl::MagFilter::Nearest);
	float normal_data[12] = { 0.5,0.5,1, 0.5,0.5,1, 0.5,0.5,1, 0.5,0.5,1 };
	default_normal.SetImage(0, gl::PixelDataInternalFormat::Rgb, 2, 2, gl::PixelDataFormat::Rgb, gl::PixelDataType::Float, normal_data);
	default_normal.SetWrapFilter(gl::WrapMode::Repeat, gl::WrapMode::Repeat, gl::MinFilter::Nearest, gl::MagFilter::Nearest);
	...
	auto p3t2_vs = RsrcMngr<gl::Shader>::Instance().GetOrCreate(p3t2_vs_path, gl::ShaderType::VertexShader, p3t2_vs_path);
	auto p3t2n3t3_vs = RsrcMngr<gl::Shader>::Instance().GetOrCreate(p3t2n3t3_vs_path, gl::ShaderType::VertexShader, p3t2n3t3_vs_path);
	auto p2t2_vs = RsrcMngr<gl::Shader>::Instance().GetOrCreate(p2t2_vs_path, gl::ShaderType::VertexShader, p2t2_vs_path);
	auto env_vs = RsrcMngr<gl::Shader>::Instance().GetOrCreate(env_vs_path, gl::ShaderType::VertexShader, env_vs_path);
	auto stdBRDF_fs = RsrcMngr<gl::Shader>::Instance().GetOrCreate(stdBRDF_fs_path, gl::ShaderType::FragmentShader, stdBRDF_fs_path);
	auto light_fs = RsrcMngr<gl::Shader>::Instance().GetOrCreate(light_fs_path, gl::ShaderType::FragmentShader, light_fs_path);
	auto img_fs = RsrcMngr<gl::Shader>::Instance().GetOrCreate(img_fs_path, gl::ShaderType::FragmentShader, img_fs_path);
	auto deferred_lighting_fs = RsrcMngr<gl::Shader>::Instance().GetOrCreate(deferred_lighting_fs_path, gl::ShaderType::FragmentShader, deferred_lighting_fs_path);
	auto env_fs = RsrcMngr<gl::Shader>::Instance().GetOrCreate(env_fs_path, gl::ShaderType::FragmentShader, env_fs_path);
	auto postprocess_fs = RsrcMngr<gl::Shader>::Instance().GetOrCreate(postprocess_fs_path, gl::ShaderType::FragmentShader, postprocess_fs_path);
	...
	screen = new gl::Mesh(gl::BasicPrimitiveType::Triangles, 2, 4, quad_indices, quad_vertices, { 2,2 });

	auto sphereMesh = TriMesh(TriMesh::Type::Sphere);
	sphere = new gl::Mesh(gl::BasicPrimitiveType::Triangles, sphereMesh.indices->size(), sphereMesh.positions->size(), sphereMesh.indices->data()->data(),
	...
	cube = new gl::Mesh(gl::BasicPrimitiveType::Triangles, cubeMesh.indices->size(), cubeMesh.positions->size(), cubeMesh.indices->data()->data(),
	...
	square = new gl::Mesh(gl::BasicPrimitiveType::Triangles, squareMesh.indices->size(), squareMesh.positions->size(), squareMesh.indices->data()->data(),
...
gl::Mesh* DeferredRenderer::Impl::GetPrimitiveMesh(const Primitive* primitive) {
	if (vtable_is<TriMesh>(primitive)) {
		...
		auto mesh = new gl::Mesh(gl::BasicPrimitiveType::Triangles, trimesh->indices->size(), trimesh->positions->size(), trimesh->indices->data()->data(),
...
gl::Texture2D* DeferredRenderer::Impl::GetGLTex2D(const Texture2D* tex, DefaultTex default_tex, TexPrecision precision) {
	if (tex == nullptr) {
		switch (default_tex)
		{
	...
	new_tex->SetImage(0, c2if[tex->img->channel - 1], tex->img->width, tex->img->height, c2f[tex->img->channel - 1], gl::PixelDataType::Float, tex->img->data.get());
	new_tex->SetWrapFilter(wrapmodemap[tex->wrap_u], wrapmodemap[tex->wrap_v], static_cast<gl::MinFilter>(samplemodemap[tex->sample_mode]), samplemodemap[tex->sample_mode]);
...
void DeferredRenderer::Impl::ResizeBuffer(size_t width, size_t height) {
	...
	for (auto tex : gtexs) {
		tex->SetImage(0, gl::PixelDataInternalFormat::Rgba32F, width, height, gl::PixelDataFormat::Rgba, gl::PixelDataType::Float, nullptr);
	}
	lightingBuffer_tex->SetImage(0, gl::PixelDataInternalFormat::Rgb32F, width, height, gl::PixelDataFormat::Rgb, gl::PixelDataType::Float, nullptr);
	
	depth->SetStorage(gl::FramebufferInternalFormat::DepthComponent, width, height);
	...
	lightingBuffer.Attach(gl::FramebufferAttachment::ColorAttachment0, lightingBuffer_tex);
...
void DeferredRenderer::Impl::RenderImpl(Scene* scene, SObj* camObj, size_t width, size_t height) {
	...
	stdBRDFProgram->SetMatf4("view", transformf::look_at(cam_pos, cam_pos + cam_front));
	stdBRDFProgram->SetMatf4("projection", transformf::perspective(camera->fov, camera->ar, 0.1f, 100.f));
	...
	lightProgram->SetMatf4("view", transformf::look_at(cam_pos, cam_pos + cam_front));
	lightProgram->SetMatf4("projection", transformf::perspective(camera->fov, camera->ar, 0.1f, 100.f));
	...
	scene->Each([this, &envLight, &pointLightNum, &rectLightNum](Cmpt::Light* light, Cmpt::SObjPtr* sobjptr, Cmpt::L2W* l2w) {
		if (vtable_is<PointLight>(light->light.get())) {
			...
			deferred_lightingProgram->SetVecf3((obj + ".position").c_str(), l2w->WorldPos());
			deferred_lightingProgram->SetVecf3((obj + ".radiance").c_str(), pointLight->intensity * pointLight->color);
		}
			...
			if (geo) {
				if (vtable_is<Square>(geo->primitive.get())) {
					...
					deferred_lightingProgram->SetVecf3((obj + ".radiance").c_str(), areaLight->color * areaLight->intensity);
				}
			}
		}
	});
	...
	deferred_lightingProgram->SetUInt("pointlight_num", static_cast<GLuint>(pointLightNum));
	deferred_lightingProgram->SetUInt("rectlight_num", static_cast<GLuint>(rectLightNum));
	...
	scene->Each([this](Cmpt::Geometry* geo, Cmpt::Material* mat, Cmpt::L2W* l2w) {
		...
		if (vtable_is<stdBRDF>(mat->material.get())) {
			...
			stdBRDFProgram->Active(3, GetGLTex2D(brdf->normal_map, DefaultTex::Normal));
		}
	...
	scene->Each([this](Cmpt::Geometry* geo, Cmpt::Light* light, Cmpt::L2W* l2w) {
		...
		if (vtable_is<AreaLight>(light->light.get())) {
			auto arealight = static_cast<const AreaLight*>(light->light.get());
			lightProgram->SetVecf3("radiance_factor", arealight->color * arealight->intensity);
	...
	deferred_lightingProgram->Active(0, gb.GetTex2D(gl::FramebufferAttachment::ColorAttachment0));
	deferred_lightingProgram->Active(1, gb.GetTex2D(gl::FramebufferAttachment::ColorAttachment1));
	deferred_lightingProgram->Active(2, gb.GetTex2D(gl::FramebufferAttachment::ColorAttachment2));
	deferred_lightingProgram->Active(3, gb.GetTex2D(gl::FramebufferAttachment::ColorAttachment3));
	...
	if (envLight)
		envProgram->Active(0, GetGLTex2D(envLight->texture, DefaultTex::White, TexPrecision::F32));
	...
	postprocessProgram->Active(0, lightingBuffer.GetTex2D(gl::FramebufferAttachment::ColorAttachment0));
